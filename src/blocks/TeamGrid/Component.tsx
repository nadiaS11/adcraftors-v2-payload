"use client"

import React from "react"
import { motion } from "framer-motion"
import { cn } from "@/utilities/ui"
import { fadeInUp, staggerContainer, defaultViewport, cardHover } from "@/lib/animations/variants"
import { Media } from "@/components/Media"
import type { TeamGridBlock as TeamGridBlockType, TeamMember } from "@/payload-types"

type Props = TeamGridBlockType & {
  members?: TeamMember[]
}

export const TeamGridComponent: React.FC<Props> = ({ header, settings, members = [] }) => {
  const columns = settings?.columns || "4"
  const style = settings?.style || "cards"
  const showBio = settings?.showBio ?? true
  const showSocialLinks = settings?.showSocialLinks ?? true
  const showDepartment = settings?.showDepartment ?? false

  const gridCols = {
    "3": "md:grid-cols-2 lg:grid-cols-3",
    "4": "md:grid-cols-2 lg:grid-cols-4",
    "5": "md:grid-cols-3 lg:grid-cols-5",
  }

  return (
    <section className="py-16 md:py-24">
      <div className="container mx-auto px-4">
        {header?.eyebrow && (
          <motion.span
            initial="hidden"
            whileInView="visible"
            viewport={defaultViewport}
            variants={fadeInUp}
            className="inline-block text-sm font-semibold text-primary uppercase tracking-wider mb-3"
          >
            {header.eyebrow}
          </motion.span>
        )}

        {header?.headline && (
          <motion.h2
            initial="hidden"
            whileInView="visible"
            viewport={defaultViewport}
            variants={fadeInUp}
            className="text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4"
          >
            {header.headline}
          </motion.h2>
        )}

        {header?.description && (
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={defaultViewport}
            variants={fadeInUp}
            className="text-lg text-muted-foreground mb-12 max-w-2xl"
          />
        )}

        <motion.div
          className={cn("grid gap-6 md:gap-8", gridCols[columns as keyof typeof gridCols])}
          initial="hidden"
          whileInView="visible"
          viewport={defaultViewport}
          variants={staggerContainer}
        >
          {members.map((member, index) => (
            <motion.div
              key={member.id}
              variants={fadeInUp}
              whileHover={cardHover}
              className={cn(
                "group relative",
                style === "cards" &&
                  "bg-card rounded-2xl overflow-hidden border border-border hover:border-primary/30 transition-all duration-300",
                style === "photos" && "rounded-2xl overflow-hidden",
                style === "hover" && "rounded-2xl overflow-hidden",
              )}
            >
              {style === "cards" && (
                <TeamCard
                  member={member}
                  showBio={showBio}
                  showSocialLinks={showSocialLinks}
                  showDepartment={showDepartment}
                />
              )}

              {style === "photos" && <PhotoOnlyCard member={member} />}

              {style === "hover" && <HoverRevealCard member={member} showBio={showBio} />}
            </motion.div>
          ))}
        </motion.div>
      </div>
    </section>
  )
}

type TeamCardProps = {
  member: TeamMember
  showBio?: boolean
  showSocialLinks?: boolean
  showDepartment?: boolean
}

const TeamCard: React.FC<TeamCardProps> = ({
  member,
  showBio,
  showSocialLinks,
  showDepartment,
}) => {
  const socialIcons: Record<string, React.ReactNode> = {
    linkedin: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
      </svg>
    ),
    twitter: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
    dribbble: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0c-6.628 0-12 5.373-12 12s5.372 12 12 12 12-5.373 12-12-5.372-12-12-12zm9.885 11.441c-2.575-.422-4.943-.445-7.103-.073-.244-.563-.497-1.125-.767-1.68 2.31-1 4.165-2.358 5.548-4.082 1.35 1.594 2.197 3.619 2.322 5.835zm-3.842-7.282c-1.205 1.554-2.868 2.783-4.986 3.68-1.016-1.861-2.178-3.676-3.488-5.438.779-.197 1.591-.314 2.431-.314 2.275 0 4.368.779 6.043 2.072zm-10.516-.993c1.331 1.742 2.511 3.538 3.537 5.381-2.43.715-5.331 1.082-8.684 1.105.692-2.835 2.601-5.193 5.147-6.486zm-5.44 8.834l.013-.256c3.849-.005 7.169-.448 9.95-1.322.233.475.456.952.67 1.432-3.38 1.057-6.165 3.222-8.337 6.48-1.432-1.719-2.296-3.927-2.296-6.334zm3.829 7.81c1.969-3.088 4.482-5.098 7.598-6.027.928 2.42 1.609 4.91 2.043 7.46-3.349 1.291-6.953.666-9.641-1.433zm11.586.43c-.438-2.353-1.08-4.653-1.92-6.897 1.876-.265 3.94-.196 6.199.196-.437 2.786-2.028 5.192-4.279 6.701z" />
      </svg>
    ),
    github: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
      </svg>
    ),
  }

  return (
    <>
      <div className="relative aspect-square overflow-hidden">
        {member.photo && typeof member.photo === "object" && (
          <Media
            resource={member.photo}
            className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
          />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />

        {showSocialLinks && member.socialLinks && (
          <div className="absolute bottom-4 left-4 right-4 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {Object.entries(member.socialLinks).map(([platform, url]) =>
              url ? (
                <a
                  key={platform}
                  href={url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-10 h-10 rounded-full bg-white/90 text-foreground flex items-center justify-center hover:bg-primary hover:text-white transition-colors"
                >
                  {socialIcons[platform] || socialIcons["linkedin"]}
                </a>
              ) : null,
            )}
          </div>
        )}
      </div>

      <div className="p-5">
        <h3 className="text-lg font-bold text-foreground group-hover:text-primary transition-colors">
          {member.name}
        </h3>
        <p className="text-sm text-primary font-medium mb-2">{member.role}</p>

        {showDepartment && member.department && (
          <span className="text-xs font-medium px-2 py-1 bg-muted text-muted-foreground rounded-full">
            {member.department}
          </span>
        )}

        {showBio && member.bio && (
          <p className="text-sm text-muted-foreground mt-3 line-clamp-3">{member.bio}</p>
        )}
      </div>
    </>
  )
}

const PhotoOnlyCard: React.FC<{ member: TeamMember }> = ({ member }) => (
  <div className="relative aspect-square overflow-hidden group">
    {member.photo && typeof member.photo === "object" && (
      <Media
        resource={member.photo}
        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
      />
    )}
    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
    <div className="absolute bottom-0 left-0 right-0 p-4">
      <h3 className="text-lg font-bold text-white">{member.name}</h3>
      <p className="text-sm text-white/80">{member.role}</p>
    </div>
  </div>
)

const HoverRevealCard: React.FC<{
  member: TeamMember
  showBio?: boolean
}> = ({ member, showBio }) => (
  <div className="relative aspect-square overflow-hidden">
    {member.photo && typeof member.photo === "object" && (
      <Media
        resource={member.photo}
        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
      />
    )}
    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
    <div className="absolute inset-0 flex flex-col justify-end p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      <h3 className="text-lg font-bold text-white">{member.name}</h3>
      <p className="text-sm text-primary mb-2">{member.role}</p>
      {showBio && member.bio && (
        <p className="text-sm text-white/80 line-clamp-3 mb-3">{member.bio}</p>
      )}
    </div>
  </div>
)
